Feature: FastAPI deps and router wiring
  ...
  I want the readiness gate and manual-trigger endpoint to behave correctly
  ...
  Scenario: _ready raises 503 when rabbitmq is not connected
    Given app state with init done, no error, but rabbitmq not connected
    When I call _ready expecting an error
    Then a 503 HTTPException is raised

  Scenario: get_client, get_tracked_lines and get_publisher return the state objects
    Given app state with init done and no error
    When I call the dependency getters
    Then each getter returns the matching state object

  Scenario: POST /poll returns the polled count
  ...
  Scenario: POST /poll surfaces handler errors as a 500

...
def _to_dataclass(row: RecipientModel) -> Recipient:
    return Recipient(id=row.id, phone_number=row.phone_number, name=row.name, active=row.active)
...
class RecipientsRepository:
    ...
    def create(self, phone_number: str, name: Optional[str]) -> Recipient:
        with TransactionManager.get().session() as s:
            row = RecipientModel(phone_number=phone_number, name=name)
            s.add(row)
            s.flush()
            s.refresh(row)

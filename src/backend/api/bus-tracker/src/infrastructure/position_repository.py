...
class PositionRepository:
    def insert(
        ...
        """Idempotent insert: the SPPO feed re-sends the same ping across
        consecutive polls, so (vehicle_id, captured_at) is the natural
        dedup key — a repeat is silently dropped rather than duplicated."""
    ...
    def find_latest(self, line_code: str) -> list[dict]:
        """Most recent position per vehicle for the given line."""
        with TransactionManager.get().read_only() as s:
            rows = (
                s.query(BusPositionModel)
                .filter(BusPositionModel.line_code == line_code)

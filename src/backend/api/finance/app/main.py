...
def _init(app: FastAPI) -> None:
    try:
        ...
        bus = get_event_bus()
        bus.subscribe(TransactionRecorded, lambda e: logger.info(f"TransactionRecorded id={e.transaction_id} type={e.type} amount={e.amount}"))
        bus.subscribe(TransactionDeleted, lambda e: logger.info(f"TransactionDeleted id={e.transaction_id}"))
        app.state.repo = repo
        app.state.bus = bus
